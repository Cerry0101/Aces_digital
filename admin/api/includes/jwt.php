...
function validateJWT ( $token=null, $assoc=false ) {
	...
	if ( $signature !== $valid ) throw new Exception("Unauthorized", 401);
...
function validateHTTPAuthorization() {
	if ( ! isset($_SERVER['HTTP_AUTHORIZATION']) ) throw new Exception("Unauthorized", 401);

	list($schema,$token) = explode(' ',$_SERVER['HTTP_AUTHORIZATION']);
	return validateJWT( str_replace('\/','/',$token) );
}
